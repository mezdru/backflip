...
exports.unsubscribeCallback = async (req, res, next) => {
  try {
    let userToUnsub = await User.findOne({
      "email.hash": req.params.hash,
      "email.token": req.params.token
    });
    if (!userToUnsub)
      return res.render("emails/unsubscribe", {
        success: false,
        errorMessage: res.__(
          "User not found. Please <a href='mailto:contact@wingzy.com'>contact us</a>"
        )
      });
    ...
    if (process.env.NODE_ENV === "production")
      SlackHelper.notify(
        ...
        "An User (" +
          userToUnsub._id +
          ") unsubscribe from auto-tranctionnal emails (" +
          userToUnsub.loginEmail +
          ")"
      );
    return res.render("emails/unsubscribe", {
      success: true,
      userEmail: userToUnsub.loginEmail
    });
  } catch (e) {
    console.error(e);
    return res.render("emails/unsubscribe", {
      success: false,
      errorMessage: res.__(
        "An unexpected error occured. Please <a href='mailto:contact@wingzy.com'>contact us</a>"
      )
    });
  }
};
...
exports.sendSecurityNotification = async (req, res, next) => {
  ...
  EmailUser.sendNewIntegrationEmail(
    req.user,
...
exports.sendAskConfirmation = async (req, res, next) => {
  await EmailUser.sendEmailConfirmation(
    req.user,
    res,
    req.params.orgTag
  ).catch(err => next(err));
  ...
  Agenda.scheduleJobWithTiming("sendEmailConfirmation", {
    userId: req.user._id,
    orgTag: req.params.orgTag
  });
  ...
  User.findOne({ "email.normalized": User.normalizeEmail(req.body.userEmail) })
    .then(user => {
      if (!user) {
        req.backflip = {
          status: 404,
          message: "User not found with this email : " + req.body.userEmail
        };
        return next();
      }
...
exports.sendHelpRequest = async (req, res, next) => {
  ...
  let mailjetRes = await EmailHelper.emailHelpRequest(
    ...
    new UrlHelper(
      helpRequest.organisation.tag,
      "profile/" + helpRequest.sender.tag,
      null,
      req.user.locale
    ).getUrl(),
    helpRequest.sender,
...
exports.sendSkillsProposition = async (req, res, next) => {
  ...
  let recipientUser = await User.findOne({
    "orgsAndRecords.record": sp.recipient._id
  }).catch(e => null);
  ...
  req.backflip = {
    status: 200,
    message: "Skills proposition sent with success."
  };
  return next();
};
