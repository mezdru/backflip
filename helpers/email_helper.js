...
exports.emailReactivateUser = (recipient, organisation, firstName, url, urlUnsub, tips, i18n) => {
  return send(
    recipient,
    (firstName ? i18n.__("{{firstName}}, we miss you!", { firstName: firstName }) : i18n.__("We miss you.")),
    {
      ...
      "orgBannerUrl": (organisation && organisation.cover ? organisation.cover.url || defaultBannerUrl : defaultBannerUrl),
      "orgLogoUrl": (organisation && organisation.logo ? organisation.logo.url || defaultLogoUrl : defaultLogoUrl),
      "tagline": i18n.__("Find the right person at the right time within %s at %s", organisation && organisation.name ? organisation.name : 'your company', url || defaultLink),
      "outro": i18n.__("Got any question? feedback? advise? Contact us! <a href='mailto:contact@wingzy.com'>contact us.</a><br/><a href='{{unsubLink}}'>Click here to unsubscribe.</a>", { unsubLink: urlUnsub })
    },
    '854412'
  );
}

exports.emailSecurityIntegration = (recipient, integrationName, integrationUserEmail, url, res) => {
  return send(
    recipient,
    res.__("Your {{integrationName}} account is linked to Wingzy", { integrationName: integrationName }),
    {
...
exports.emailConfirmationInvitation = (recipient, organisation, firstName, invitationUrl, orgUrl, res) => {
  return send(
    recipient,
...
exports.emailInvitationAccepted = (recipientName, recipient, senderName, organisation, url, res) => {
  return send(
    recipient,
...
exports.emailSpread = (recipient, senderName, senderEmail, organisationName, url, text, res) => {
  return send(
    recipient,
...
exports.emailReinvite = (recipient, senderName, senderEmail, organisationName, url, res) => {
  return send(
    recipient,
...
exports.emailInvite = (recipient, senderName, senderEmail, organisationName, customMessage, url, res) => {
  return send(
    recipient,
    res.__("Join %s on Wingzy", organisationName),
    {
...
exports.emailIncompleteProfile = (recipient, missingFields, recordUrl, organisation, res) => {
  return send(
    recipient,
    "-",
    {
      "title": "-",
      "text": "-",
      "ctaText": res.__("Update your profile"),
      "squareIcon": "https://emojis.wiki/emoji-pics/twitter/hugging-face-twitter.png",
      "ctaUrl": recordUrl,
      "orgBannerUrl": organisation && organisation.cover ? organisation.cover.url || defaultBannerUrl : defaultBannerUrl,
      "orgLogoUrl": organisation && organisation.logo ? organisation.logo.url || defaultLogoUrl : defaultLogoUrl,
